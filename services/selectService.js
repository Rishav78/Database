...
async function select(req, res, query) {
    ...
    let info = {query,d,location}, records = '';
    let readRecordsModule = spawn('python3', [path.join(__dirname, 'PyModule', 'readRecords.py'), JSON.stringify(info)]);
    readRecordsModule.stdout.on('data', chunk => records += chunk);
    readRecordsModule.stdout.on('end', () => res.json(JSON.parse(records)));
    // let records;
